...
// Type for lightweight vocab set (from cached getVocabSets)
interface VocabSetLight {
  id: string;
  name: string;
...
interface ManagePageClientProps {
  initialVocabSets: VocabSetLight[];
}

export function ManagePageClient({ initialVocabSets }: ManagePageClientProps) {
  const [vocabSets, setVocabSets] = useState<VocabSetLight[]>(initialVocabSets ?? []);
  ...
  const [loading, setLoading] = useState(initialVocabSets.length === 0);
    ...
    return vocabSets.reduce((sum, set) => {
      return (
        ...
        set.words.reduce((wordSum, word) => {
          // Lightweight words from initial data don't have examples
          const examples = (word as Partial<VocabWord & { examples: VocabExample[] }>).examples;
          return wordSum + (examples?.length ?? 0);
        }, 0)
      );
    }, 0);
  }, [vocabSets]);
