...
export async function PrintMatchingContent({ params }: PageProps) {
  const { setId } = await params;

  let vocabSet;
  try {
    vocabSet = await prisma.vocabSet.findUnique({
      where: { id: setId },
      include: {
        words: {
          select: {
    ...
    // If database is not available at build time, return empty state
    vocabSet = null;
  }
